...
async def ask_settings(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text(text='Привет! Настраиваем рассылку!'
...
async def mailing_settings(message: Message, state: FSMContext, bot: Bot):
    ...
    if mailing.strip().lower() == 'да':
        await asyncio.sleep(0.2)
        await message.answer('Выбери город для утренней рассылки погоды или введи свой', reply_markup=cities_kb)
        ...
        try:
            ...
            await backup_db.add_settings(tg_id=message.from_user.id, mailing=0, city='NULL')
...
async def city_settings(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        if len(await get_weather_today(city)) != 0:
            ...
            await backup_db.add_settings(tg_id=message.from_user.id, mailing=1, city=city)
            await message.answer(text='Запомнил!\n'
                                      'Настройки в любой момент можно изменить😉',
                                 reply_markup=menu_kb)
            await state.clear()
        else:
            await message.answer(text=weather_error + '\nПопробуем еще?',
                                 reply_markup=again)
    except Exception as error:

...
async def mailing_settings(
    ...
    user = get_user(session=session, tg_id=callback.from_user.id)
...
async def set_mailing_city_with_cb(
    ...
    async with aiohttp.Session() as http_session:
        weather = get_weather_today(http_session=http_session, city=city)
    if weather:
        user_update = UserUpdate(
            tg_id=callback.from_user.id,
            mailing=True,
...
async def set_mailing_city(message: Message, state: FSMContext):
    ...
    session = await db_helper.get_scoped_session()
    user = get_user(session=session, tg_id=message.from_user.id)
    async with aiohttp.Session() as http_session:
        weather = get_weather_today(http_session=http_session, city=city)
    if weather:
        user_update = UserUpdate(
